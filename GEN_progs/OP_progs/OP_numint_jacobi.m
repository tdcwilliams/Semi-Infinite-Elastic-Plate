function [x,w]=OP_numint_jacobi(alf,bet,N)
...
for j=1:N
  z_unf(j,1)=...
	cos( pi*( (N+1-j)-0.25+0.5*alf)/(N+0.5*(alfbet+1)) );
end
...
if (its == MAXIT+1) then
  ...
  disp('too many iterations in "OP_numint_jacobi')
  disp(' ')
end

%OUTPUTS
x=z;
if 0%%original formula, but unstable for large N
  w=gamma(alf+N)*gamma(bet+N)/gamma(N+1)/...
       gamma(N+alf+bet+1)*temp*2^alfbet./(pp.*p2);
else
  v0=1:N-1;
  facvec=(v0+alf).*(v0+bet)./(v0+1)./(v0+alf+bet+1);
  fac=prod(facvec)*gamma(1+alf)*gamma(1+bet)/gamma(2+alf+bet);
  w=fac*temp*2^alfbet./(pp.*p2);
end