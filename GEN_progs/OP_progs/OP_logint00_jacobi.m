function Y=OP_logint00_jacobi(tt,alf,bet)
...
if ~isempty(j0)
  Ngl=50;
  [uu,ww]=OP_numint_laguerre(bet,Ngl);
  IG0=ig0(tt(j0),uu,alf,bet);
  int1=IG0*ww;
  int2=IG0*(uu.*ww);
  err=10*TOL;
  while err>TOL
  ...
  t=tt(j0);
  Y(j0)=(1+t).^(bet+1).*( log(1+t).*int1 - int2 );
end
