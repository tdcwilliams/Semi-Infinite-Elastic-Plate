...
methods
   function obj = SingleInterfaceCompGal(...
      ...
      obj.period = phys_vars{1};
      obj.theta_inc = phys_vars{2};
      obj.H_dim = phys_vars{3};
      obj.N_poly = NN(1);
      obj.edge_condition = bc;
      obj.FAST_KERNEL = 0;
      obj.INC_SUB = 1;
      ...
      if length(NN)==1
        ...
        obj.N_roots = NN(2);
        Ninput = obj.N_roots*ones(1,1+INC_SUB);
      end
      if INC_SUB==1
         [Rts,rts,HH,...
            obj.alp_y,obj.del0,obj.L] =...
               NDmakeZ2_sub(phys_vars,hh,Ninput,youngs);
      else
         [Rts,rts,H,obj.alp_y,obj.del0,obj.L] =...
      ...
      obj.rhs_info.roots = Rts{1};
      obj.rhs_info.k_x = rts{1};
      ...
      obj.h_ref = obj.rhs_info.thickness;
      obj.E_ref = obj.rhs_info.young;

      %% common to both sides
      obj.nu = NDphyspram(5);
      obj.rho_wtr = NDphyspram(3);
      obj.rho_ice = NDphyspram(4);
      obj.omega = 2*pi/obj.period
      obj.lam = obj.del0(1);
      obj.mu = -obj.del0(2);
      obj.nu1 = (1-obj.nu)*obj.alp_y^2;

      %% get order of singularity
      obj.mvec = (0:obj.N_poly)';
      obj.alpC = .5-1/3*obj.INC_SUB*(hh(1)~=hh(2));%%=1/6 if submergence included;
   ...
   function info = extend_info(obj,info,hmax)
      ...
      info.mu_lame = info.young/2/(1+obj.nu);
      info.Kc_dim  = info.young*info.thickness/(1-obj.nu^2);
         %%compressional rigidity ~ Pa*m ~ rho_wtr*om^2*L^2*L
      info.Kc = info.Kc_dim/(obj.rho_wtr*obj.omega^2*obj.L^2)/obj.L;
      info.m_dim   = obj.rho_ice*info.thickness;
      info.sig_dim = info.m_dim/obj.rho_wtr;
      info.zc_dim  = -info.sig_dim+info.thickness/2;

      %% non-dimensionalise
      info.sig = info.sig_dim/obj.L;
      info.zc = info.zc_dim/obj.L;

      %% wave numbers for compressional problem
      if info.Kc==0
         info.kc_dim = 0;
      else
         info.kc_dim = sqrt(obj.om^2*info.m_dim/info.Kc_dim);
      end
      info.kc = info.kc_dim*obj.L

      %% rotational inertia
      info.J_dim = obj.rho_ice*info.thickness^3/12;

      %%make real parts of gam be >0 (for M0_wtr,M1_wtr)
      jneg = find(real(info.roots)<0);
      info.roots(jneg) = -info.roots(jneg);

      info.hr = info.thickness/obj.h_ref;
      info.Dr = info.young/obj.E_ref*info.hr^3;

      %% coefficients in expansion of Green's function's derivatives
      info.BGzz = calc_res(...
         {obj.lam,obj.mu,info.depth_nondim},...
         info.Dr,info.hr,...
         info.roots).*info.roots./info.k_x;%%BG_0^(0)
      info.Lam  = info.Dr*info.roots.^4+obj.lam-obj.hr*obj.mu;
      info.BGz  = info.Lam.*info.BGzz;
      info.BG   = info.Lam.*BGz;%%BG_0^(2)

      %% inner products of polynomials with eigenfunctions
      kap = -1i*info.roots.info.depth_nondim;
      [KAP,MU] = meshgrid(kap,2*obj.mvec+obj.alpC);
      besJ = besselj(MU1,KAP1);%% length(mvec) x length(kap1)
      c_left = gamma(obj.alpC)*(obj.alpC+2*obj.mvec).*(-1).^obj.mvec;
      c_right = (2./kap).^obj.alpC./cos(kap);
      info.F = diag(c_left)*besJ*diag(c_right);%%same as (37)

      %%
      info.fm = info.roots.^2-obj.nu1;
      info.fp = info.roots.^2+obj.nu1;
   ...
   function obj = assemble(obj)
      ...
      obj.dzc_dim  = obj.lhs_info.zc_dim+...
                        -obj.rhs_info.zc_dim;%pause
      obj.dzc = obj.dzc_dim/L;
      obj.H = obj.rhs_info.depth_nondim+obj.rhs_info.sig;

      obj.M1_rel = rho_wtr*obj.omega^2*obj.L^4;
         %%scale bending moment by this, since
         %%\int\sig_ij.(z-z_c).dz ~ Pa*m^2 ~ rho_wtr*om^2*L^2*L^2 = D1/L
      obj.M0_rel = rho_wtr*obj.omega^2*obj.L^3;
      ...
      if 1%~FAST_KERNEL%%basic way:-other ways don't work
         MK = obj.lhs_info.F*diag(1i*obj.lhs_info.BG)*obj.lhs_info.F.'+...
      ...
      M1_wtr = M1a+M1b-(obj.H+obj.rhs_info.zc)*M0_wtr;

      %%contribution from side integrals to rr2:
      obj.rr_wtr_psi2 = -2i*obj.lhs_info.BG.*M1_wtr;
         %%Q2 (psi(0+)) column
      obj.rr_wtr_U2 = -2i*obj.lhs_info.BG.*M0_wtr;
         %%U2 (u(0+)) column
      Minc_tot = obj.lhs_info.num_inc_waves+...
                  + obj.rhs_info.num_inc_waves;%%only 4 possible incident waves here;
      obj.j_side_ints = Minc_tot+[4 6];%%col's corresponding to psi(0+), u(0+)
      `
      %%matrix M0w=M_M0w*rr2
      obj.M_M0w = -M0_wtr.';%%\int\sig_11 dz=\int(-P)dz=-\int\phi dz
      obj.M_M1w = -M1_wtr.';%%\int\sig_11*(z-zc1)dz=\int(-P)*(z-zc1)dz=-\int\phi*(z-zc1)dz
      ...
      obj.forcing_u = [obj.lhs_info.F(:,1:obj.lhs_info.num_inc_waves),...
                   ZM,...
                   ...
                   obj.rhs_info.ME,...
                   ZM,ZM];
      obj.forcing_u(:,j_side_ints) = obj.forcing_u(:,j_side_ints)+...
         +obj.lhs_info.F*[obj.rr_wtr_psi2,obj.rr_wtr_U2];
   end%% end assemble
   ...
   function obj = solve(obj)
      ...
      obj.solution.u = obj.MK\obj.forcing_u;
      ...
      %%from side integrals;
      obj.solution.lhs.fluid_coeffs(:,obj.j_side_ints) =...
         + obj.solution.lhs.fluid_coeffs(:,obj.j_side_ints)+...
         + [obj.rr_wtr_psi2,obj.rr_wtr_U2];
      % ================================================
...
if INC_SUB==1
   ...
   [Rts,rts,H,alpy,del0,L]  = NDmakeZ2_rel(phys_vars,hh,Ninput,youngs);
   HH                       = [H H];
end
...
%%
%jside    = [5 7];%%col's corresponding to psi(0+), u(0+)
Minc_tot = 4;%%only 4 possible incident waves here;
jside    = Minc_tot+[4 6];%%col's corresponding to psi(0+), u(0+)
...
if Dr(1)==0%%water on left:
   %j_dis2   = [2 3 4 6]; %% OLD: Ninc=1
   j_dis2   = [jc_inc1,j_Q1,j_U1,j_Q2(1)];
   jc_inc1  = [];
      %% no more [S(0-) (Ninc+1),psi(0-) (Ninc+2),u(0-) (Ninc+5)];
      %% S(0+)=0 (Ninc+3);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% scattering matrices
%% NB rearrange so 1st line and column are for compressive wave
%% (if it exists)
if Dr(1)~=0
   jj = [jc_inc1,j_inc1];
