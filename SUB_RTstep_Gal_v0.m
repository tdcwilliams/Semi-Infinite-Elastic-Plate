...
%func  = -[1i*F1(:,1),ME1,ME2];func(1:10,:)
uu    = -MK\[1i*F1(:,1),ME1,ME2];%uu(1:10,:)
%%
rr          = 2*diag(BG1)*F1.'*uu;%rr(1:10,:)
rr(1)       = rr(1)+1;
rr(:,2:3)   = rr(:,2:3)-2*diag(BGz1)*E1;%tstr=[rn,rr*[1;P1]]
%rr(1:10,:)
%%
tt          = -2*diag(BG2)*F2.'*uu;
tt(:,4:5)   = tt(:,4:5)+2*diag(BGz2)*E2;%tstt=[tn tt*[1;P1]]
%%
...
if do_test
   ...
   disp([Rm Tm]);
   %%
   if 0%%rerun program with thicknesses reversed to check
